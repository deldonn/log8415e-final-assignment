...
def _ensure_ssh_rule(ec2, sg_id: str):
    """Ensure SSH access rule (port 22 from 0.0.0.0/0) exists in the security group."""
    try:
...
def _add_sg_rule_if_missing(ec2, sg_id: str, port: int, source_sg_id: str, description: str):
    """Add a security group rule allowing traffic from another SG (SG-to-SG rule)."""
    try:
        response = ec2.describe_security_groups(GroupIds=[sg_id])
        sg = response["SecurityGroups"][0]
...
def get_sg_names() -> dict:
    """Get security group names for the project (prefixed with project name)."""
    config = get_config()
...
def create_gatekeeper_security_group() -> str:
    ...
    Rules: Port 8080 + Port 22 open to Internet.
...
def create_proxy_security_group(gatekeeper_sg_id: str = None) -> str:
    ...
    Rules: Port 8000 open ONLY from Gatekeeper SG + Port 22 for SSH.
    """
    config = get_config()
    ec2 = get_ec2_client()
    sg_names = get_sg_names()
...
def create_db_security_group(proxy_sg_id: str = None) -> str:
    ...
    Rules: Port 3306 open ONLY from Proxy SG + Port 22 + inter-node traffic.
    """
    ec2 = get_ec2_client()
    sg_names = get_sg_names()
    sg_name = sg_names["db"]
...
def create_all_security_groups() -> dict:
    """Create all security groups in dependency order: Gatekeeper -> Proxy -> DB."""
...
def delete_security_groups():
    """Delete all project security groups in reverse dependency order."""
    ec2 = get_ec2_client()
    vpc_id = get_default_vpc_id()
    sg_names = get_sg_names()
...
def get_security_group_id(sg_type: str) -> str:
    """Get security group ID by type ('db', 'proxy', 'gatekeeper')."""
    sg_names = get_sg_names()
